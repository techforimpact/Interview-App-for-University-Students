...
import com.example.newapp.Adapter.StudentApprovalAdapter
...
class PendingRequestFragment : Fragment() {
    ...
    private var studentAdapter: StudentApprovalAdapter? = null
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_pending_request, container, false)


        val hr = arguments?.getParcelable<HR>("hr")

        profileImage = view.findViewById(R.id.hr_pending_profile_picture)

        Picasso.get().load(hr!!.getImage()).placeholder(R.drawable.profile_picture).into(profileImage)

        recyclerView = view.findViewById(R.id.recycler_view_pending_recruiter)
        recyclerView?.setHasFixedSize(true)
        recyclerView?.layoutManager = LinearLayoutManager(context)

        mStudents = ArrayList()

        studentAdapter = StudentApprovalAdapter(requireContext(), mStudents as ArrayList<Student>, true)
        ...
        query.addValueEventListener(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (snap in snapshot.children) {
                    ...
                    if (student != null && student.getUniversity().contains(hr.getName(), ignoreCase = true) && student.getApproved() == "pending") {
